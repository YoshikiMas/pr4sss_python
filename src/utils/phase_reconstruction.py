...
def divmisi_ver2(spec1, spec2, mixture, stft, istft, maxiter=50, gamma=0.5, lm=1e4, epsilon=1e-15):
    ...
    u1 = np.zeros_like(spec1);
    u2 = np.zeros_like(spec2);
    ...
    f1est = istft(prox_dis(x1, amp1, gamma))
    f2est = istft(prox_dis(x2, amp2, gamma))
    return f1est, f2est